...
// }
  ...
  if (insertedCandidatesIds.length) {
    const reSourcesIds = [];
    for (const candId of insertedCandidatesIds) {
      logFunction(
        candId,
        candidateCreatedby,
        `Candidate mapped `,
      ...
      const candObj = candidates.find(c => c.candidatesId === candId);
      if (candObj?.resumeSource) {
        reSourcesIds.push(candObj.resumeSource);
      }
    }
