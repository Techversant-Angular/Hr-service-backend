...
exports.addProgressV1 = tryCatch(async (req, res) => {
  ...
  if (created) {
    ...
    logFunction(candidate.serviceCandidate, progressAssignee, 'Scores and Feedback added technical 1', 3);
    return res
      .status(200)
      .json({ result: true, message: "Technical Progress added" });
  }
...
exports.progressDetail = tryCatch(async (req, res) => {
  ...
  if (candidates) {
    let [skills, metadata] = await sequelize.query(
